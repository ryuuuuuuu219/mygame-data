...
$stageRows = @(
    ...
    @("M01", 2500, 300, 5700, 2800, 8200, "target: LASM x3 / non-target: AA_GUN x36, SAM x18"),
    @("MVP", "selectedstage依存", 300, 5700, "selectedstage依存", "selectedstage依存", "M01と同配置"),
    @("M02", 3000, 2000, "6,500 + 条件付きUAV最大 4,500", 5000, "9,500 + 条件付きUAV最大 4,500", "UAV_STORAGE target x2 / non-target x1 / fighter最大x45"),
    @("M03", 3500, 4100, 4100, 7600, 7600, "ace_m03 x4, fighter_m03_special x1"),
    @("M04", 4000, 5000, 8700, 9000, 12700, "AIR_BATTLESHIP core / JAMMER / 追加SAM x20 / 追加AA_GUN x16"),
    @("M05", 4500, 1600, 1600, 6100, 6100, "Railgun x4, HR_AAGUN x12")
)
...
$enemyRows = @(
    ...
    @("fighter_m03_special / AceM03Special", 100, 100, ""),
...
$weaponRows = @(
    ...
    @("GUN", 1488, "発射レート", 1, 60, "+0.1", 590, 138800),
    @("GUN", 1488, "射程", 450, 1000, "+10", 55, 138800),
    @("GUN", 1488, "威力", 3, 150, "+1", 147, 138800),
    @("GUN", 1488, "当たり判定サイズ", 3, 8, "+0.05", 100, 138800),
    @("GUN", 1488, "弾数", 120, 2000, "+5", 376, 138800),
    @("GUN", 1488, "初速", 100, 1200, "+5", 220, 138800),
...
$memoRows = @(
    ...
    @("M01/MVP は非ターゲットの AA_GUN と SAM 合計で 5,400 点。"),
